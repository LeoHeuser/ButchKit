...
public struct ExternalPackage: Identifiable, Sendable {
    ...
    /// gets the same reaction and none of them can forget to wire it. The app's array of packages
    /// is usually a static constant, so this reaches static API, such as an SDK's own entry
    /// points, rather than an instance the app holds in its state.
