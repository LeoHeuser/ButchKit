...
- Privacy Policy plus Terms of Service when both URLs are configured. Policies open in `GatedWebView` inside the sheet. StoreKit draws them with the subscription controls and nowhere else, so under the lifetime products ButchKit draws the same two links itself: a paywall without a group has them too.
...
- A lifetime product the user already owns shows a checkmark in place of its price and cannot be bought again. VoiceOver reads the optional `purchasedLabel`.
...
row. Inside `sheet`, the `restoreOffline` and `productsUnavailable` groups and `purchasedLabel` are optional. A later ButchKit
